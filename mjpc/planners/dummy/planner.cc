...
void dummyPlanner::Initialize(mjModel* model, const Task& task) {
...
void dummyPlanner::Reset(int horizon, const double* initial_repeated_action) {
  // state
...
// set state
void dummyPlanner::SetState(const State& state) {
  state.CopyTo(this->state.data(), this->mocap.data(), this->userdata.data(),
               &this->time);
}

void dummyPlanner::UpdateNumTrajectoriesFromGUI() {
  num_trajectory_ = mju_min(num_rollouts_gui_, kMaxTrajectory);
}

// optimize nominal policy using dummy
void dummyPlanner::OptimizePolicy(int horizon, ThreadPool& pool) {
...
void dummyPlanner::NominalTrajectory(int horizon, ThreadPool& pool) {
...
void dummyPlanner::ActionFromPolicy(double* action, const double* state,
...
const Trajectory* dummyPlanner::BestTrajectory() {
  const std::shared_lock<std::shared_mutex> lock(mtx_);
  return &policy.trajectory;
}

// visualize planner-specific traces in GUI
void dummyPlanner::Traces(mjvScene* scn) {}

// planner-specific GUI elements
void dummyPlanner::GUI(mjUI& ui) {
  mjuiDef defdummy[] = {
  ...
  mju::sprintf_arr(defdummy[0].other, "%i %i", 1, kMaxTrajectory);

  // add dummy planner
  mjui_add(&ui, defdummy);
}

// planner-specific plots
void dummyPlanner::Plots(mjvFigure* fig_planner, mjvFigure* fig_timer,
...
// single dummy iteration
void dummyPlanner::Iteration(int horizon, ThreadPool& pool) {
...
void dummyPlanner::ActionRollouts(int horizon, ThreadPool& pool) {
...
void dummyPlanner::FeedbackRollouts(int horizon, ThreadPool& pool) {
  int count_before = pool.GetCount();
  for (int i = 0; i < num_trajectory_; i++) {
    pool.Schedule([&data = data_, &trajectory = trajectory,
                   &candidate_policy = candidate_policy,
