...
// allocate memory
void dummyPolicy::Allocate(const mjModel* model, const Task& task, int horizon) {
  // model
  this->model = model;
...
void dummyPolicy::Reset(int horizon, const double* initial_repeated_action) {
...
void dummyPolicy::Action(double* action, const double* state,
                        double time) const {
  // dimension
...
// copy policy
void dummyPolicy::CopyFrom(const dummyPolicy& policy, int horizon) {
  // reference
